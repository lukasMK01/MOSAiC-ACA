...
for num in range(len(eagle)):
    ...
    for i in range(0,len(arr_640)):
        time_ls_old.append(datetime.fromisoformat(date_640+" "+start_time_640) + (k+i)* timedelta(seconds=0.05))
    ...
    '''
    def clip_arr(input1, input2, wvlarr1, wvlarr2, minpix1, maxpix1, minpix2, maxpix2, jump):
        new_arr1 = [[0 for _ in range(len(wvlarr1))] for _ in range(len(input1))]
        for j in range(len(wvlarr1)):
            for i in range(len(input1)):
                new_arr1[i][j] = np.mean(input1[i, minpix1:maxpix1, j])
                
        new_arr2 = [[0 for _ in range(len(wvlarr2[11:]))] for _ in range(len(input2))]
        for j in range(len(wvlarr2[11:])):
            for i in range(len(input2)):
                new_arr2[i][j] = np.mean(input2[i, minpix2:maxpix2, 11+j])
        '''
    ...
    if create_nc == True:
        ...
        ncfile = netCDF4.Dataset('Flight_'+ datee +'_'+timee+'_EagleHawk_2Pixelrows_Radiances_interp.nc', mode='w', format='NETCDF4')
        ...
        if createplot == True:
            ...
            create_plots(data_arr = rad1, time_arr = time[:]-time[0], wavel_arr = wvl, times = times_to_plot, wavel = wvl_to_plot, date_of_flight=datee, time_of_flight=timee)
            create_plots(data_arr = rad2, time_arr = time[:]-time[0], wavel_arr = wvl, times = times_to_plot, wavel = wvl_to_plot, date_of_flight=datee, time_of_flight=timee)
